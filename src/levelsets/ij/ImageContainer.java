...
public class ImageContainer {
	...
	public ImageContainer(final ImagePlus ip) {
		...
		if (isStack) {
			for (int i = 0; i < stack_size; i++) {
				sproc[i] = (ShortProcessor) ip.getStack().getProcessor(i + 1)
	...
	private ImageContainer(final ShortProcessor[] iproc) {
		...
		for (int i = 0; i < stack_size; i++) {
			sproc[i] = (ShortProcessor) iproc[i].duplicate().convertToShort(
					false);
			proc_pixels[i] = (short[]) sproc[i].getPixels();
		}
	...
	public final ImagePlus createImagePlus(final String title) {
		...
		if (isStack) {
			final ImageStack is = new ImageStack(this.getWidth(),
					this.getHeight());
			for (int i = 0; i < stack_size; i++) {
				is.addSlice("", sproc[i]);
			}
	 ...
	 *            The Z coordinate
	 * @param pixel
	 *            An array suitable to be filled with pixel values - usually
	 *            RGB(A).
	 ...
	 *            The Z coordinate
	 * @param pixel
	 *            An array containing values for the pixel - usually RGB(A).
	 */
	public final void setPixel(final int x, final int y, final int z,
			final int pixel) {
		if (isStack) {
	 ...
	 * 
	 * @param index
	 *            The index (zero based indices) of the image to copy
	...
	/**
	 * Sets the image stack to the contents of the passed three-dimensional
	 * double value array. The values are scaled to grey values between 0 and
	 * 255
	 * 
	 * @param data
	 *            The data array
		...
		for (int i = 0; i < data.length; i++) {
			for (int j = 0; j < data[0].length; j++) {
				for (int k = 0; k < data[0][0].length; k++) {
					if (data[i][j][k] == Double.MAX_VALUE)
						continue;
		...
		for (int z = 0; z < this.getImageCount(); z++) {
			...
			for (int x = 0; x < this.getWidth(); x++) {
				for (int y = 0; y < this.getHeight(); y++) {
					if (data[x][y][z] == Double.MAX_VALUE)
						continue;
					// int current =
					// (int)Math.round(((int)(Math.abs(data[x][y][z])) + add) *
					// scaler);
					this.setPixel(x, y, z, (int) Math.round(((int) (Math
							.abs(data[x][y][z])) + add) * scaler));
				}
			}
		}
	}
	...
	/**
	 * Calculates grey value gradients of the image container an returns a
	 * result array.
	 * 
	...
	public final double[][][] calculateGradients() {
		...
		final double[][][] gradients = new double[this.getWidth()][this
				.getHeight()][this.getImageCount()];

		for (int z = 0; z < gradients[0][0].length; z++) {
			for (int x = 1; x < gradients.length - 1; x++) {
				for (int y = 1; y < gradients[0].length - 1; y++) {

					final double xGradient = (this.getPixel(x + 1, y, z) - this
							.getPixel(x - 1, y, z)) / 2;
					final double yGradient = (this.getPixel(x, y + 1, z) - this
							.getPixel(x, y - 1, z)) / 2;
					final double zGradient;
					if ((z > 0) && (z < gradients[0][0].length - 1)) {
						zGradient = (this.getPixel(x, y, z + 1) - this
								.getPixel(x, y, z - 1)) / (2 * zScale);
					} else {
						zGradient = 0;
					}
					gradients[x][y][z] = Math.sqrt(xGradient * xGradient
							+ yGradient * yGradient + zGradient * zGradient);
				}
			}
		}
	 ...
	 *            The Filter
	 * @param start
	 *            The start index (zero based indices)
	 * @param end
	 *            The end index (zero based indices)
	 */
	public final void applyFilter(final Filter filter, final int start,
			final int end) {
		IJ.log("Applying filter : " + filter.getClass().getCanonicalName()
				+ " (on images " + start + " to " + end + ")");
		if (this.sproc == null)
			return;
		...
		for (int i = start; i <= end; i++) {
			...
			// ij2bufferedImage(sproc[i].createImage());
			// final BufferedImage filtered_image = filter.filter(image);
			// sproc[i] = new ShortProcessor(filtered_image);

			// THIS HAS TO CHANGE: it creates a new ShortProcessor from a
			// BufferedImage that is create from the createImage() of another
			// ShortProcessor !
			// AND in the filter subclasses, all sorts of unholy java imaging
			// code goes on.
			// sproc[i] = new
			// ShortProcessor(filter.filter(ij2bufferedImage(sproc[i].createImage())));
	...
	public final BufferedImage ij2bufferedImage(final Image im) {
		final BufferedImage bi = new BufferedImage(im.getWidth(null),
				im.getHeight(null), BufferedImage.TYPE_USHORT_GRAY);
