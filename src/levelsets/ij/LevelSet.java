...
public class LevelSet implements PlugIn {
	...
	private static boolean test_dialog = false, test_algorithm = false,
			test_roi = false;
	...
	static public void main(String args[]) {
		...
		if (test_roi) {
			...
			ls_filter.showROI(testIP, new PolygonRoi(poly_rx, poly_ry,
					poly_rx.length, PolygonRoi.POLYGON), null);
		}
		if (test_algorithm) {
	 ...
	 * @param with_progress
	 *            Whether to show an image that tracks progress
	 */
	public ImagePlus execute(ImagePlus imp, boolean with_progress) {

		// TODO Would make sense to offer starting points as mask in separate
		// image.
		// If no ROI found, have the additional dialog field to select mask
		// image.
		...
		ImageProgressContainer progressImage = with_progress ? new ImageProgressContainer()
				: null;
		if (null != progressImage) {
			progressImage.duplicateImages(ic);
			progressImage.createImagePlus("Segmentation progress of "
					+ imp.getTitle());
		...
		sc_roi.setROI(roi, ic.getWidth(), ic.getHeight(), ic.getImageCount(),
				imp.getCurrentSlice());
		...
		try {
			// Fast marching
			if (fast_marching) {
				final FastMarching fm = new FastMarching(ic, progressImage,
						sc_roi, true, (int) fm_grey, fm_dist);
				IJ.log("Fast Marching: Starting "
			...
			// Level set
			if (level_sets) {
				IJ.log("Level Set (" + levelsetList[ls_choice] + "): Starting "
						+ new Date(System.currentTimeMillis()));
				IJ.log("Note: Each iteration step is " + ITER_INC
						+ " iterations");

				LevelSetImplementation ls = lf.getImplementation(
						levelsetList[ls_choice], ic, progressImage, sc_ls);
	...
	public void run(String arg) {
		...
		// if never called before set the parameters to meaningful defaults from
		// the class
	...
	public boolean showDialog() {
		...
		for (Iterator<Parameter> it = parameters.keySet().iterator(); it
				.hasNext();) {
			...
			if (pv instanceof Double) {
				gd.addNumericField(parameters.get(pn),
						((Double) lf.getParameterValue(pn)).doubleValue(), 2);
			}
		}
		...
		gd.addNumericField("Convergence", ((Double) lf
				.getParameterValue(Parameter.CONVERGENCE)).doubleValue(), 4);
		gd.addChoice("Region expands to ", expansionList,
				expansionList[expansion_choice]);
		gd.addMessage("");
		...
		// this.shapeStack =
		// WindowManager.getImage(stackIDs[((Choice)gd.getChoices().get(0)).getSelectedIndex()]);
		...
		lf.setParameterValue(Parameter.CONVERGENCE,
				new Double(gd.getNextNumber()));
		...
		if (test_dialog) {
			IJ.log("Convergence to "
					+ lf.getParameterValue(Parameter.CONVERGENCE));
			IJ.log("Fast Marching enabled = " + fast_marching);
			IJ.log("Fast Marching: fm_grey=" + fm_grey + " , fm_dist="
					+ fm_dist);
			IJ.log("Level Sets enabled = " + level_sets);
			IJ.log("Level Sets: w_adv= "
					+ lf.getParameterValue(Parameter.W_ADVECTION) + ", w_curv="
					+ lf.getParameterValue(Parameter.W_CURVATURE) + ", w_gray="
					+ lf.getParameterValue(Parameter.TOL_GRAYSCALE)
	...
	public void showROI(ImagePlus ip, Roi roi, StateContainer sc_in) {
		...
		final ImageProgressContainer progress = new ImageProgressContainer(
				new ImageContainer(ip));
		...
		if (roi != null) {
			...
			sc_test.setROI(roi, progress.getWidth(), progress.getHeight(),
					progress.getImageCount(), 1);
		}
		if (sc_test == null) {
	...
	private final void addEnablerListener(final Checkbox master,
			final Component[] enable, final Component[] disable) {
