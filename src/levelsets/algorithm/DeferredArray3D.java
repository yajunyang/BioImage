...
public abstract class DeferredArray3D {
	 ...
	 *            Size in Z direction
	 * @param tilesize
	 *            The tile size - length of an edge tile.
	 */
	public DeferredArray3D(final int xdim, final int ydim, final int zdim,
			final int tilesize) {
	 ...
	 */
	protected final Object getTile(final int x, final int y, final int z,
	...
	/**
	 * Dumps the array content into a textfile. A matrix is dumped for every Z
	 * layer.
	 * 
	 * @param path
	...
	public final void dumpToFile(final String path) {
		try {
			...
			out.write(this.getXLength() + " " + this.getYLength() + " "
					+ this.getZLength());
			out.newLine();
			out.newLine();
	...
	private final void checkBounds(final int x, final int y, final int z) {
		if (x < 0 || x > (xdim - 1) || y < 0 || y > (ydim - 1) || z < 0
				|| z > (zdim - 1)) {
