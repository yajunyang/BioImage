...
public class Enclose implements PlugIn, Measurements {
	...
	public void run(String arg) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (img != null) {
				...
				int allPix = 0;
				for (int k = 0; k < stats.histogram.length; k++)
					allPix = allPix + stats.histogram[k];
				if ((stats.histogram[0] + stats.histogram[255]) != allPix) {
					// IJ.error("Will threshold now. " + filename +
					// "is not an 8-bit binary image.");
				...
				if (skip == false) {
					...
					double perimeter = rt.getValue(ResultsTable.PERIMETER,
							cr - 1);
					double circul = (perimeter * perimeter) / area;
					rt.addValue("Circularity", perimeter == 0.0 ? 0.0 : 4.0
		...
		IJ.register(Enclose.class);
	}

	private PolygonRoi getPolygon(ImageProcessor ip) {
		/*
		 * This long-winded method determines the vertices of a polygon which
		 * forms a convex hull around the image. It does this by first mapping
		 * the vertical contours of the image. (Horizontal or vertical contours
		 * could have been used, but only one is necessary.) After they are
		 * mapped, all of the points which can be enclosed by connecting any
		 * other points are eliminated. This leaves only the outermost points,
		 * the ones that form the convex hull. Returns a polygon created with
		 * these points.
		 */
		...
		// Map farthest left pixel, from bottom. We've completed encircled the
		// pixel area now
		do {
			y--;
			ip.setRoi(index, y - 1, 1, 1);
		...
		// a greater slope than all of the lines after it. If this is not the
		// case,
		// then it replaces the endpoint with the endpoint of the next contour
		// pixel, and checks again, until it has removed all of the contour
		// points
		// that are concave.
		int ab = 0;
		do {
			...
			ab++;
			// IJ.showMessage(new Integer(ab).toString() + " " + new
			// Integer(ad).toString());
		...
		index = 1;
		do {
			if (polyXCoordinate[index] == polyXCoordinate[index - 1]
					&& polyYCoordinate[index] == polyYCoordinate[index - 1]) {
		...
		return new PolygonRoi(polyXCoordinate, polyYCoordinate, aa + 1, img,
				Roi.POLYGON);
	...
	private void GetDimensions(ImageProcessor ip) {
		/*
		 * Determine the dimensions of the image, and sets some of the important
		 * variables.
		 */
		...
		// start at the leftmost edge, move to the middle, and find the first
		// coloured pixel for the left edge
		this.left = -1;
		do {
			this.left++;
		...
		// This part of the methods finds the coordinates of the top, bottom,
		// right and left pixels,
