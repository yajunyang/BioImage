...
public class Frangi<T extends RealType<T>> implements PlugIn {
	...
	public static class VesselnessCalculator<T extends RealType<T>> implements
			Callable<Image<FloatType>> {
		...
		public VesselnessCalculator(final Image<T> input, float[] spacing,
				int scaleIndex, MultiTaskProgress progress) {
		...
		public Image<FloatType> call() throws Exception {
			...
			/*
			 * The cursors may go outside the image, in which case we supply
			 * mirror values:
			 */

			OutOfBoundsStrategyMirrorFactory osmf = new OutOfBoundsStrategyMirrorFactory<T>();

			LocalizableByDimCursor<T> cursor = input
					.createLocalizableByDimCursor(osmf);

			ImageFactory<FloatType> floatFactory = new ImageFactory<FloatType>(
					new FloatType(), new ArrayContainerFactory());

			Image<FloatType> resultImage = floatFactory.createImage(input
					.getDimensions());
			LocalizableByDimCursor<FloatType> resultCursor = resultImage
			...
			/*
			 * Two cursors for finding points around the point of interest, used
			 * for calculating the second derivatives at that point:
			 */

			LocalizableByDimCursor<T> ahead = input
					.createLocalizableByDimCursor(osmf);
			LocalizableByDimCursor<T> behind = input
					.createLocalizableByDimCursor(osmf);
			...
			while (cursor.hasNext()) {
				...
				for (int m = 0; m < numberOfDimensions; ++m)
					for (int n = 0; n < numberOfDimensions; ++n) {
						...
						float firstDerivativeA = (ahead.getType()
								.getRealFloat() - behind.getType()
								.getRealFloat())
								/ (2 * spacing[m]);
						...
						float firstDerivativeB = (ahead.getType()
								.getRealFloat() - behind.getType()
								.getRealFloat())
								/ (2 * spacing[m]);

						double value = (firstDerivativeA - firstDerivativeB)
								/ (2 * spacing[n]);
				...
				// Now find the eigenvalues and eigenvalues of the Hessian
				// matrix:
				EigenvalueDecomposition e = hessian.eig();

				/*
				 * Nonsense involved in sorting this array of eigenvalues by
				 * their absolute values:
				 */

				double[] eigenvaluesArray = e.getRealEigenvalues();
				ArrayList<Float> eigenvaluesArrayList = new ArrayList<Float>(
						eigenvaluesArray.length);
				...
				if (numberOfDimensions == 2) {
					...
					if (l2 <= 0 && l3 <= 0)
						v = (1 - Math.exp(an / ad)) * Math.exp(bn / bd)
								* (1 - Math.exp(cn / cd));
	...
	public ImagePlus process(ImagePlus input, int scales, double minimumScale,
			double maximumScale) {
		return process(input, scales, minimumScale, maximumScale, false, false,
				false, null);
	}
	...
	public ImagePlus process(ImagePlus input, int scales, double minimumScale,
			double maximumScale, boolean showGaussianImages,
			boolean showFilteredImages, boolean showWhichScales,
		...
		List<VesselnessCalculator<T>> calculators = new ArrayList<VesselnessCalculator<T>>();

		/*
		 * Smooth the input image with a Gaussian convolution at each scale.
		 * Also create a VesselnessCalculator object for each scale and add it
		...
		for (int scaleIndex = 0; scaleIndex < scales; ++scaleIndex) {
			...
			GaussianConvolutionReal<T> gauss = new GaussianConvolutionReal<T>(
					image, new OutOfBoundsStrategyMirrorFactory<T>(), sigmas);

			gauss.setNumThreads(processors);

			IJ.showStatus("Running Gaussian convolution at scale "
					+ formatReal(currentScale) + "...");

			/*
			 * FIXME: add some facility to report progress of the Gaussian
			 * convolution.
			 */

			if (!gauss.checkInput() || !gauss.process()) {
				IJ.error("Gaussian Convolution failed: "
						+ gauss.getErrorMessage());
				return null;
			}
			...
			if (showGaussianImages) {
				ImagePlus gaussianSmoothedImage = ImageJFunctions
						.copyToImagePlus(result);
				gaussianSmoothedImage
						.setTitle("Gaussian smoothed images at scale "
								+ scaleIndex);
				gaussianSmoothedImage.show();
			}

			VesselnessCalculator<T> calculator = new VesselnessCalculator<T>(
					result, newSpacing, scaleIndex, progress);
		...
		/*
		 * Actually run all the VesselnessCalculators with invokeAll, which
		...
		try {
			...
			/*
			 * We never call interrupt on these threads, so this should never
			 * happen...
			 */
		}

		/*
		 * We need to get() on each Future to check if there was an exception
		 * thrown:
		 */
		...
		/*
		 * Get the images from each VesselnessCalculator, displaying them if
		 * showFilteredImages was specified:
		 */
		...
		for (VesselnessCalculator vc : calculators) {
			...
			if (showFilteredImages) {
				ImagePlus imagePlusVersion = ImageJFunctions
						.copyToImagePlus(image);
		...
		/*
		 * Now we combine all the vesselness images from each scale, by taking
		 * the maximum value at each point in the image across all the
		 * vesselness images
		 */
		...
		Image<FloatType> resultImage = floatFactory.createImage(image
				.getDimensions());
		LocalizableCursor<FloatType> resultCursor = resultImage
				.createLocalizableCursor();

		ImageFactory<ByteType> byteFactory = new ImageFactory<ByteType>(
				new ByteType(), new ArrayContainerFactory());
		...
		ArrayList<LocalizableCursor<FloatType>> cursors = new ArrayList<LocalizableCursor<FloatType>>();
		...
		while (resultCursor.hasNext()) {
			...
			if (showWhichScales)
				whichCursor.fwd();
			/*
			 * If bestScale remains 0, that indicates that all images had NaN as
			...
			if (showWhichScales)
				whichCursor.getType().set(
						bestScale == 0 ? 0 : (float) minimumScale
								+ (bestScale - 1) * (float) increment);
		...
		if (showWhichScales) {
			ImagePlus whichImagePlus = ImageJFunctions.copyToImagePlus(
					whichImage, ImageJFunctions.GRAY32);
		...
		ImagePlus resultImagePlus = ImageJFunctions
				.copyToImagePlus(resultImage);
	...
	public static class Progress implements MultiTaskProgress {
		...
		public Progress(int totalTasks) {
			tasksProportionsDone = new ArrayList<Double>();
	...
	public void run(String ignored) {
		...
		ImagePlus result = process(imagePlus, scales, minimumScale,
				maximumScale, showGaussianImages, showFilteredImages,
				showWhichScales, progress);
