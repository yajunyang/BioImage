...
/**
 * A version of the Kuwahara filter that uses linear kernels rather than square
 * ones.
 *
...
public class Kuwahara_LinearStructure_Filter_v3 implements PlugInFilter {
	...
	ImageStack createKernel(int size, int nAngles) {
		...
		ImagePlus impLine = NewImage.createShortImage("imLine", sizeTemp,
				sizeTemp, 1, NewImage.FILL_BLACK);
		...
		ImagePlus impLineRotated = NewImage.createShortImage("imLineRot",
				sizeTemp, sizeTemp, 1, NewImage.FILL_BLACK);
		...
		for (iAngle = 0; iAngle < nAngles; iAngle++) {
			...
			for (x1 = nB; x1 < sizeTemp - nB; x1++) {
				for (y1 = nB; y1 < sizeTemp - nB; y1++) {
					rotLineStack[iAngle][i] = (short) ipLineRotated.getPixel(
							x1, y1);
					i++;
				}
			}
	...
	public void filter(ImageProcessor ipData, ImageStack imsKernels) {
		...
		ipData.resetMinAndMax(); // this is important because "ip.getMin();"
									// returns the smallest displayed(!!)
									// number.
		...
		for (x1 = 0; x1 < imW; x1++) {
			for (y1 = 0; y1 < imH; y1++) {
				im[x1][y1] = ipData.getPixel(x1, y1) - imMin; // substraction of
																// the minimum
																// (offset) is
																// necessary for
																// the poisson
		...
		for (int iKernel = 0; iKernel < nKernels; iKernel++) {
			...
			if (criterionMethod == 0)
				calculateCriterionVariance(imSum, imSumOfSquares, kernelSum,
						value, criterion);
			else if (criterionMethod == 1)
				calculateCriterionVarianceDivMean(imSum, imSumOfSquares,
						kernelSum, value, criterion);
			else if (criterionMethod == 2)
				calculateCriterionVarianceDivMean2(imSum, imSumOfSquares,
						kernelSum, value, criterion);
			KuwaharaGM(value, criterion, pixelsKernel, resultTemp,
					resultCriterionTemp);
		...
		putFloat2Image(ipData, result, imMin); // add also the minimum back to
												// the image to avoid that the
												// offset shifts between images
												// of a stack
	...
	// - the criterion value that was used for selection is stored in
	// "resultCriterion"
	// - this allows to compare the result with further, other Kuwahara filters

	void KuwaharaGM(float[][] value, float[][] criterion, short[] pixelsKernel,
			float[][] result, float[][] resultCriterion) {
		int x1min, x1max, y1min, y1max;
		int x2min, x2max, y2min, y2max;
	...
	void setResultAndCriterion(float[][] result, float[][] resultTemp,
			float[][] resultCriterion, float[][] resultCriterionTemp) {
	...
	public final void calculateCriterionVariance(int[][] imSum,
			int[][] imSumOfSquares, int kernelSum, float[][] value,
			float[][] criterion) {
		for (int x1 = 0; x1 < imW; x1++) {
			for (int y1 = 0; y1 < imH; y1++) {
				value[x1][y1] = (float) imSum[x1][y1] / kernelSum;
				criterion[x1][y1] = imSumOfSquares[x1][y1] / kernelSum
						- value[x1][y1] * value[x1][y1];
			}
		}
	}

	public final void calculateCriterionVarianceDivMean(int[][] imSum,
			int[][] imSumOfSquares, int kernelSum, float[][] value,
			float[][] criterion) {
		for (int x1 = 0; x1 < imW; x1++) {
			for (int y1 = 0; y1 < imH; y1++) {
				...
				criterion[x1][y1] = (imSumOfSquares[x1][y1] / kernelSum - value[x1][y1]
						* value[x1][y1])
						/ (value[x1][y1] + Float.MIN_VALUE);
			}
		}
	}

	public final void calculateCriterionVarianceDivMean2(int[][] imSum,
			int[][] imSumOfSquares, int kernelSum, float[][] value,
			float[][] criterion) {
		for (int x1 = 0; x1 < imW; x1++) {
			for (int y1 = 0; y1 < imH; y1++) {
				...
				criterion[x1][y1] = (imSumOfSquares[x1][y1] / kernelSum - value[x1][y1]
						* value[x1][y1])
						/ (value[x1][y1] * value[x1][y1] + Float.MIN_VALUE);
			}
		}
	}
	...
	boolean showDialog() {
		String[] criteria = { "Variance", "Variance / Mean",
				"Variance / Mean^2" };
		...
		// must be ODD!! this is the length of the line along which the
