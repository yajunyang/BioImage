...
public class TracerThread extends SearchThread {
	...
	public TracerThread(ImagePlus imagePlus, float stackMin, float stackMax,
			int timeoutSeconds, long reportEveryMilliseconds, int start_x,
			int start_y, int start_z, int goal_x, int goal_y, int goal_z,
			boolean reciprocal, boolean singleSlice, ComputeCurvatures hessian, // ¼ÆËãÇúÂÊ
			double multiplier, float[][] tubeness, boolean useHessian) {
		...
		SearchNode s = createNewNode(start_x, start_y, start_z, 0,
				estimateCostToGoal(start_x, start_y, start_z, true), null,
				OPEN_FROM_START);
		addNode(s, true);

		SearchNode g = createNewNode(goal_x, goal_y, goal_z, 0,
				estimateCostToGoal(goal_x, goal_y, goal_z, false), null,
				OPEN_FROM_GOAL);
		...
		if (useHessian) {
			...
			if (tubeness == null) {
				...
				if (singleSlice) {
					...
					boolean real = hessian.hessianEigenvaluesAtPoint2D(new_x,
							new_y, true, hessianEigenValues, false, true,
							x_spacing, y_spacing);
					...
					boolean real = hessian.hessianEigenvaluesAtPoint3D(new_x,
							new_y, new_z, true, hessianEigenValues, false,
							true, x_spacing, y_spacing, z_spacing);
	...
	float estimateCostToGoal(int current_x, int current_y, int current_z,
		...
		double distance = Math.sqrt(xdiff * xdiff + ydiff * ydiff + zdiff
				* zdiff);

		// System.out.println("estmateCostToGoal distance = " + distance);
