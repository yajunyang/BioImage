...
package yang.plugin.tracing.tracing;
...
public abstract class SearchThread extends Thread implements SearchInterface {
	...
	protected void reportPointsInSearch() {
		for (SearchProgressCallback progress : progressListeners)
			progress.pointsInSearch(this, open_from_start.size()
	...
	protected SearchNode createNewNode(int x, int y, int z, float g, float h,
			SearchNode predecessor, byte searchStatus) {
	...
	public void requestStop() {
		if (verbose)
			System.out
					.println("requestStop called, about to enter synchronized");
		synchronized (this) {
			if (verbose)
		...
		if (verbose)
			System.out.println("requestStop finished (threadStatus now "
					+ threadStatus + ")");
	}
	...
	public void pauseOrUnpause() {
		...
		if (verbose)
			System.out
					.println("pauseOrUnpause called, about to enter synchronized");
		synchronized (this) {
			if (verbose)
				System.out.println("... entered synchronized");
			switch (threadStatus) {
			case PAUSED:
				if (verbose)
					System.out
							.println("paused, going to switch to running - interrupting first");
				this.interrupt();
				if (verbose)
	...
	public SearchThread(ImagePlus imagePlus, float stackMin, float stackMax,
			boolean bidirectional, boolean definedGoal, boolean startPaused,
	...
	public void printStatus() {
		System.out.println("... with " + open_from_start.size()
				+ " open nodes at the start");
		System.out.println(" ... and " + closed_from_start.size()
				+ " closed nodes at the start");
		if (bidirectional) {
			System.out.println("... with " + open_from_goal.size()
					+ " open nodes at the goal");
			System.out.println(" ... and " + closed_from_goal.size()
	...
	public void run() {
		try {
			...
			if (verbose)
				printStatus();
			if (verbose)
				System.out.println("... was asked to start it in the "
						+ (startPaused ? "paused" : "unpaused") + " state.");
			...
			long started_at = lastReportMilliseconds = System
					.currentTimeMillis();
			...
			while ((open_from_start.size() > 0)
				...
				if (0 == (loops % 1000)) {
					...
					long millisecondsSinceStart = currentMilliseconds
							- started_at;

					if ((timeoutSeconds > 0)
							&& (millisecondsSinceStart > (1000 * timeoutSeconds))) {
						if (verbose)
				...
				PriorityQueue<SearchNode> open_queue = fromStart ? open_from_start
						: open_from_goal;
				PriorityQueue<SearchNode> closed_queue = fromStart ? closed_from_start
						: closed_from_goal;

				SearchNode[][] nodes_as_image_this_search = fromStart ? nodes_as_image_from_start
						: nodes_as_image_from_goal;
				SearchNode[][] nodes_as_image_other_search = fromStart ? nodes_as_image_from_goal
				...
				if (definedGoal && atGoal(p.x, p.y, p.z, fromStart)) {
					...
					if (fromStart)
						...
						foundGoal(p.asPathReversed(x_spacing, y_spacing,
				...
				p.searchStatus = fromStart ? CLOSED_FROM_START
						: CLOSED_FROM_GOAL;
				closed_queue.add(p);
				...
				for (int zdiff = -1; zdiff <= 1; zdiff++) {
					...
					if (nodes_as_image_this_search[new_z] == null) {
						nodes_as_image_this_search[new_z] = new SearchNode[width
								* height];
					}
					...
					for (int xdiff = -1; xdiff <= 1; xdiff++)
						for (int ydiff = -1; ydiff <= 1; ydiff++) {
							...
							double xdiffsq = (xdiff * x_spacing)
									* (xdiff * x_spacing);
							double ydiffsq = (ydiff * y_spacing)
									* (ydiff * y_spacing);
							double zdiffsq = (zdiff * z_spacing)
									* (zdiff * z_spacing);

							float h_for_new_point = estimateCostToGoal(new_x,
									new_y, new_z, fromStart);

							double cost_moving_to_new_point = costMovingTo(
									new_x, new_y, new_z);
							if (cost_moving_to_new_point < minimum_cost_per_unit_distance) {
								cost_moving_to_new_point = minimum_cost_per_unit_distance;
							}

							float g_for_new_point = (float) (p.g + Math
									.sqrt(xdiffsq + ydiffsq + zdiffsq)
									* cost_moving_to_new_point);

							float f_for_new_point = h_for_new_point
									+ g_for_new_point;

							SearchNode newNode = createNewNode(new_x, new_y,
									new_z, g_for_new_point, h_for_new_point, p,
									FREE);

							// Is this newNode really new?
							SearchNode alreadyThereInThisSearch = nodes_as_image_this_search[new_z][new_y
									* width + new_x];
							...
							if (alreadyThereInThisSearch == null) {
								...
								addingNode(newNode);
								nodes_as_image_this_search[new_z][new_y * width
										+ new_x] = newNode;
							...
							if (bidirectional
									&& nodes_as_image_other_search[new_z] != null) {

								SearchNode alreadyThereInOtherSearch = nodes_as_image_other_search[new_z][new_y
										* width + new_x];
								if (alreadyThereInOtherSearch != null) {
									...
									if (alreadyThereInOtherSearch != null
											&& (alreadyThereInOtherSearch.searchStatus == CLOSED_FROM_START || alreadyThereInOtherSearch.searchStatus == CLOSED_FROM_GOAL)) {

										if (fromStart) {
											result = p.asPath(x_spacing,
													y_spacing, z_spacing,
													spacing_units);
											Path fromGoalReversed = alreadyThereInOtherSearch
													.asPathReversed(x_spacing,
															y_spacing,
															z_spacing,
															spacing_units);
											result.add(fromGoalReversed);
										} else {
											result = alreadyThereInOtherSearch
													.asPath(x_spacing,
															y_spacing,
															z_spacing,
															spacing_units);
											result.add(p.asPathReversed(
													x_spacing, y_spacing,
													z_spacing, spacing_units));
										}
										if (verbose)
			...
			if (verbose)
				System.out
						.println("FAILED to find a route.  Shouldn't happen...");
			...
			if (plane == ThreePanes.XY_PLANE) {
				...
				for (int y = 0; y < height; ++y)
					for (int x = 0; x < width; ++x) {
						SearchNode n = anyNodeUnderThreshold(x, y, z,
								drawingThreshold);
						if (n == null)
							continue;
						byte status = n.searchStatus;
						if (status == start_status || status == goal_status)
							g.fillRect(canvas.myScreenX(x) - pixel_size / 2,
									canvas.myScreenY(y) - pixel_size / 2,
				...
				for (int z = 0; z < depth; ++z)
					for (int x = 0; x < width; ++x) {
						...
						if (status == start_status || status == goal_status)
							g.fillRect(canvas.myScreenX(x) - pixel_size / 2,
									canvas.myScreenY(z) - pixel_size / 2,
									pixel_size, pixel_size);
					}
				...
				for (int y = 0; y < height; ++y)
					for (int z = 0; z < depth; ++z) {
						...
						byte status = n.searchStatus;
						if (status == start_status || status == goal_status)
							g.fillRect(canvas.myScreenX(z) - pixel_size / 2,
									canvas.myScreenY(y) - pixel_size / 2,
									pixel_size, pixel_size);
					}
			}
	...
	public void addNode(SearchNode n, boolean fromStart) {
		...
		SearchNode[][] nodes_as_image = fromStart ? nodes_as_image_from_start
				: nodes_as_image_from_goal;
