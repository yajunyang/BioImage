 ...
 */
public class Power_Law extends PlugInFrame implements ActionListener,
	...
	public void run(String arg) {
		...
		if ((type == imp.GRAY16) || (type == imp.GRAY32)) {
			...
			IJ.error(
					"Error",
					"Power Law requires an image of type\n \n8-bit grayscale\n8-bit indexed color\nRGB color\n");
			return;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == preview) {
			...
			RaiseToPower rp = new RaiseToPower(imp, plot.getLUT(),
					imp.getTitle());
	...
	private boolean checkGamma() {
		if (valueGamma <= 0) {
			IJ.error("Error",
					"You must enter a value greater\n than zero for gamma");
			return false;
		}
		if (valueGamma > 25) {
			IJ.error("Error",
					"You must enter a value less \nor equal than 25 for gamma");
			return false;
		}
		...
		if (value.indexOf('.') != -1) {
			...
			if (value.substring(value.indexOf('.')).length() > 3) {
				IJ.error("Error",
						"You must enter a value with not\n more than two decimal places");
				return false;
			}
		}
		return true;
	...
	private boolean checkScale() {
		if (valueScale <= 0) {
			IJ.error("Error",
					"You must enter a value greater than zero for scale factor");
			return false;
		}
		return true;
	}
	...
	public void adjustmentValueChanged(AdjustmentEvent e) {
		...
		if (sliderValue <= 100) {
			...
			valueGamma = ((sliderValue - 100) / 10.0f) + 1.0f;
...
class ValueViewer implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		TextWindow tw = new TextWindow("Plot Values", headings, sb.toString(),
				200, 400);
		tw.show();
	}
}
...
class InfoHelpViewer implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		TextWindow tw = new TextWindow("About \"Power Law\"", sb.toString(),
				700, 500);
		tw.show();
	}
}
...
class PowerPlot extends Canvas implements MouseMotionListener {
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < 255; i++) {
			...
			g.drawLine(first_couple[0], first_couple[1], first_couple[0],
					first_couple[1]);
			g.drawLine(second_couple[0], second_couple[1], second_couple[0],
					second_couple[1]);
			approximationCurve(g, second_couple[0], second_couple[1],
					first_couple[0], first_couple[1]);
		}
	}

	public void approximationCurve(Graphics g, int succ_x, int succ_y,
			int prev_x, int prev_y) {
		double hypo = Math.sqrt(Math.pow((prev_x - succ_x), 2)
				+ Math.pow((prev_y - succ_y), 2));
		if (hypo > 1D) {
...
class RaiseToPower {
	...
	public int[] applyTransform(int[] pixels) {
		...
		for (int i = 0; i < pixels.length; i++) {
			...
			blue = lut[blue & 0xff];
			output[i] = ((red & 0xff) << 16) + ((green & 0xff) << 8)
					+ (blue & 0xff);
		}
		return output;
	}
...
class PreviewImage implements ActionListener {
	...
	public PreviewImage(ImagePlus img, double time) {
		...
		imgWin.setSize(canvas.getSize().width + 150,
				canvas.getSize().height + 150);
		GridBagLayout gridbag = new GridBagLayout();
		GridBagConstraints c = new GridBagConstraints();
