package yang.plugin.transform.tj;
...
public class TJ_Affine implements PlugIn, ActionListener, WindowListener {
	...
	public void run(String arg) {
		...
		gd.addChoice("Interpolation:", TJ.interpolations,
				TJ.interpolations[interpolation]);
		...
		(new TJAffine()).run(image, file, interpolation, background, adjust,
				antialias);
	}
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (e.getSource() == browseButton) {
			final FileDialog fdg = new FileDialog(IJ.getInstance(), TJ.name()
...
class TJAffine {
	...
	void run(final ImagePlus image, final String file, final int interpolation,
			final String background, final boolean adjust,
		...
		try {
			...
			double bg;
			try {
				bg = Double.parseDouble(background);
			} catch (Exception e) {
			...
			case 2:
				...
				break;
			case 3:
				scheme = Affine.BSPLINE3;
				break;
			case 4:
				scheme = Affine.OMOMS3;
				break;
			case 5:
				scheme = Affine.BSPLINE5;
				break;
			}
			final Image output = affiner.run(input, transform, scheme, adjust,
					antialias);
