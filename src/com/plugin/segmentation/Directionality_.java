 ...
 * that orientation.
 * <p>
 * Angles are reported in their common mathematical sense. That is: 0 is the
 ...
 * periodic nature of the histogram. The 'Direction ()' column reports the
 * center of the gaussian. The 'Dispersion ()' column reports the standard
 ...
 * +90 orientation in the Fourier transform of the image, compared to the
...
//		data_frame.setLocation(x, y);
	...
	public ResultsTable displayResultsTable() {
		...
		for (int i = wrap_index; i < bins.length; i++) {
			...
			table.addValue("Direction (бу)", wrapped_bins[index]);
	...
	private boolean showDialog() {
		...
		gd.addNumericField("Histogram start", setting_bin_start , 0, 4, "бу");
...
//		double[] dir = new double[nbins]; // histo with #bins
		...
		final float[] kernel_y = new float[] { 
				...
				-4f, 	-3f, 	0f, 	3f, 	4f,
				-3f,  	-2f,  	0f, 	2f, 	3f,
				-2f,  	-1f,  	0f, 	1f, 	2f		} ; // That's gx, but we want to have a 90 shift, to comply to the rest of the plugin
		...
		for (int i = 0; i < pixels_gx.length; i++) {
			...
			pixels_theta[i] = (float) (angle * 180.0 / Math.PI); // deg, -90 to 90
...
//							big_hue_px[i] = (float) ( 255 *  ( ( 1 + hue_px[i]/Math.PI ) % 1 ) ); 
		...
		MouseMotionListener ml = new MouseMotionListener() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				try {
					...
					IJ.showStatus( String.format("Orientation: %5.1f бу - Amont: %5.1f %%", angle, 100*amount));
	...
	public static void main(String[] args) {
		...
		Line line_30deg 	= new Line(10.0, 412.0, 446.4102, 112.0); // 400px long line, 30
		Line line_30deg2 = new Line(10.0, 312.0, 446.4102, 12.0); // 400px long line, 30
		Line line_m60deg = new Line(10.0, 10, 300.0, 446.4102); // 400px long line, 60
		...
		System.out.println(String.format("Found maxima at %.1f, expected it at 30бу.\n", center, 30));
