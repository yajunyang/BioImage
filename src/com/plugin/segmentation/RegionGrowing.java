...
public class RegionGrowing {
		ImageStack imageStack;   // Stack off images
		
		private final int imageDepth = 256;	// number of intensity values a pixel can hava
		private byte[] mark; // mark processed foreground voxels
		private int label = 3;
		private int labels = 0;
		private int xx, yy, zz; // Maximum size in x,y and z-direction
		private int buffer_full = 0;
	    private int removed = 0;
	    private int sizeThreshold = 100; // The threshold in voxels for keeping foreground regions.
	    private boolean preserveBorderObjects = false;
	    private ByteProcessor[] images;
	    private int[] sizes = new int[imageDepth]; // Histogram that shows how many pixels a region have.
	    private boolean[] border = new boolean[imageDepth]; // stores if a region is a border region.
	    private boolean quit = false; // true if we get an overflow.
