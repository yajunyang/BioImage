...
//		System.out.println(System.getProperty("usr.home")); // null
	...
	private static void setDefaultShortcuts() {
		...
		shortcuts
				.put("Edit > Change transparency", getKeyStroke(KeyEvent.VK_T));
	...
	public static void load() {
		try {
			...
			projection = integer(properties.getProperty("Projection",
					str(projection)));
			showGlobalCoordinateSystem = bool(properties.getProperty(
					"Show_Global_Coordinate_System",
					str(showGlobalCoordinateSystem)));
			showLocalCoordinateSystemsByDefault = bool(properties.getProperty(
	...
	public static void initFromDialog(final Image3DUniverse univ) {
		final GenericDialog gd = new GenericDialog("View Preferences",
				univ.getWindow());
		...
		if (gd.getNextBoolean() && univ != null)
			((ImageCanvas3D) univ.getCanvas()).getBG().getColor(
					defaultBackground);
