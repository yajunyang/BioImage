...
public class ContentCreatorDialog {
	...
	public Content showDialog(final Image3DUniverse univ, final ImagePlus imp,
		...
		if (file != null) {
			...
			for (int i = 1; i <= img_count; i++) {
				...
				if (ip != null && ip.getWidth() != 0
						&& !ip.getTitle().equals("3d"))
		...
		final String[] types = new String[] { "Volume", "Orthoslice",
				"Surface", "Surface Plot 2D", "Multiorthoslices" };
		type = type < 0 ? 0 : type;
		...
		di.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(final ItemEvent e) {
				if (di.getSelectedIndex() == ContentConstants.SURFACE)
		...
		channels = new boolean[] { gd.getNextBoolean(), gd.getNextBoolean(),
				gd.getNextBoolean() };
		...
		if (univ.contains(name)) {
			IJ.error("Could not add new content. A content with " + "name \""
					+ name + "\" exists already.");
			return null;
		}
	...
	private Content createContent() {
		final ImagePlus[] imps = file != null ? ContentCreator.getImages(file)
		...
		final Content c = ContentCreator.createContent(name, imps, type,
				resamplingFactor, timepoint, color, threshold, channels);
		return c;
	}
